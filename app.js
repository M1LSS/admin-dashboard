...
function refreshSubstitutions() {
  const today = new Date().toLocaleDateString("en-CA");
  const subsRef = database.ref("substitutions/" + today);
  const attendanceRef = database.ref("attendance/" + today);
  ...
  Promise.all([
    subsRef.once("value"),
    attendanceRef.once("value"),
    teachersRef.once("value")
  ]).then(([subSnap, attSnap, teacherSnap]) => {
    const updates = {};
    const attendanceMap = {};
    const teacherMap = {};

    attSnap.forEach(c => attendanceMap[c.key.toUpperCase()] = c.val().status);
    teacherSnap.forEach(c => teacherMap[c.key.toUpperCase()] = c.val());

    subSnap.forEach(child => {
      const sub = child.val();
      const originalUID = (sub.original_teacherUID || "").toUpperCase();
      const status = attendanceMap[originalUID];

      if (status === "late" || status === "present") {
        updates[`substitutions/${today}/${child.key}/substitute_teacher`] = teacherMap[originalUID]?.name || "Recovered";
      }
    });
